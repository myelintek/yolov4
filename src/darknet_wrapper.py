...
DEFAULT_TRAIN_DIR='/mlsteam/data/yolo/training_data/yolo/images'
...
def train_run(pretrained_weights, output_weights=None):
    names_file = stparams.get_value('names', '/mlsteam/data/yolo/obj.names')
    ...
    model_cfg = write_model_cfg(
        ...
        max_batches=stparams.get_value('max_batches', 500),
        steps=stparams.get_value('steps', '400, 450'),
    ...
    data_cfg = write_data_cfg(
        ...
        valid_list=valid_list,
        names=names_file,
        backup=stparams.get_value('backup', '/mlsteam/data/yolo/model_weights/trained'),
        eval_method=stparams.get_value('eval', 'coco')
    )

    train(data_cfg, model_cfg, pretrained_weights=pretrained_weights)
    if output_weights:
        backup_path = stparams.get_value('backup', '/mlsteam/data/yolo/model_weights/trained')
